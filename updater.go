...
func (m *Mdgo) UpdateSection(sectionID, content string, afterLine ...string) error {
	...
	// If reading fails, we might check if it's NotExist error.
	// But m.readFile wraps it. If we use os.ReadFile directly we know errors.
	// Assuming m.readFile returns os.PathError for missing file if it's file based.
	// However, processContent handles empty content logic.
	...
	// Write back using writeIfDifferent (or m.writeFile directly)
	// writeIfDifferent checks content again, but we know it changed.
	// Use m.writeFile directly if available, else standard.
	// m.writeFile calls writeIfDifferent internally? No, explicit func.
	// Let's use m.writeFile.
	// Note: We are writing to m.inputPath, assuming in-place update.
...
func (m *Mdgo) processContent(currentContent, sectionID, content string, afterLineArgs ...string) (string, bool, error) {
...
func (m *Mdgo) findAllSections(content, startMarker, endMarker string) ([]sectionInfo, error) {
...
func (m *Mdgo) determineInsertPosition(content string, sections []sectionInfo, afterLine string) int {
	lines := strings.Split(content, "\n")
...
func (m *Mdgo) removeAllSections(content string, sections []sectionInfo) string {
	if len(sections) == 0 {
		return content
	}
	...
	// Actually scan order is safe. But let's verify. Yes, findAllSections returns in order.
	// We need reverse order to remove from indices.
...
func (m *Mdgo) insertSectionAtPosition(content, section string, position int) string {
	lines := strings.Split(content, "\n")
